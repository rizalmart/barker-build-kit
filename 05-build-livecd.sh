...
if [ "$EFI_IMG_FOUND" != "" ]; then
	...
	genisoimage -J -D -R ${VOLI} -o ${WKGBASE}/${ISONAME} ${BOOTPARM} ${BOOTCAT} -full-iso9660-filenames -no-emul-boot -boot-load-size 4 -boot-info-table -eltorito-alt-boot -eltorito-platform 0xEF -eltorito-boot ${EFI_BOOT_IMG} -no-emul-boot ${WKGBASE}/puppy-livecd-build/
else
