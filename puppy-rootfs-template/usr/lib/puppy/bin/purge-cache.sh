...
for c1 in thumbnails midori google-chrome opera mozilla/seamonkey mozilla/firefox "moonchild productions" microsoft-edge chromium slimjet iron slimjet brave maxthon waterfox vivaldi qupzilla falkon yandex qtweb gnome-web netsurf galeon xombrero kmeleon epiphany vlc wine rhythmbox media-art totem
do
...
rm -f $croot/tmp/ntfs-errors/* 2>/dev/null
...
if [ -d $croot/var/cache/tazpkg ]; then
 rm -rf $croot/var/cache/tazpkg/*     
fi

[ -d  $croot/var/cache/apt/archives ] && rm -f $croot/var/cache/apt/archives/*.deb
