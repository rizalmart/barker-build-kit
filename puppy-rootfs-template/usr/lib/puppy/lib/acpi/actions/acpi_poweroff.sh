...
SUSPEND_PROG=/usr/lib/puppy/lib/acpi/actions/suspend.sh

[ -x "$SUSPEND_PROG" ] || SUSPEND_PROG=""

ACPI_CONFIG=/etc/puppy-acpi.conf
TIME_LIMIT=""
...
[ -f "$SOUND" ] && which $(basename $PLAY) >/dev/null && env $(basename $PLAY) -D sysdefault "$SOUND"
...
if [ -n "$DISPLAY" -o -n "$WAYLAND_DISPLAY" ]; then
 ...
 if [ $TIMELIMIT -gt 0 ]; then
  ...
  _farewell=$(gettext "Power button is pushed!")
  ...
  _Click_Suspend=$(gettext "Click 'Shutdown' to shutdown right now, 'Suspend' to suspend Puppy, or 'Cancel' to continue Puppy.")
  _Susupend=$(gettext "Suspend")
  _Cancel=$(gettext "Cancel")
  _OK=$(gettext "Shutdown")
  ...
  [ "$MSG1" ] && MSG1=$(printf "$MSG1" $TIMELIMIT)
   ...
   export DIALOG='<window title="acpid"><vbox space-expand="true" space-fill="true">
  <vbox space-expand="true" space-fill="true">
  <text><label>"'$MSG' '$MSG1'"</label></text>
  <progressbar>
...
[ -x /etc/init.d/rc.acpi ] && /etc/init.d/rc.acpi stop
[ -x /etc/init.d/rc.acpid ] && /etc/init.d/rc.acpid stop
