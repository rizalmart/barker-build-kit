...
. gettext.sh
. /etc/rc.d/PUPSTATE #variables created at bootup by ${INITRD_FLD}/usr/sbin/init...
. /etc/DISTRO_SPECS
...
if [ "`whoami`" != "root" ]; then
    ...
    /usr/lib/gtkdialog/box_ok "$(gettext 'Resize Personal Storage File')" error "root access is needed to use this program"
	exit
	fi
fi
...
if [ -f ${INITRD_FLD}/tmp/no_resize2fs ] ; then #set by the initrd init script...
...
case $PUPMODE in
	"12") PERSISTMNTPT="${INITRD_FLD}/pup_rw"  ;;
	"13") PERSISTMNTPT="${INITRD_FLD}/pup_ro1" ;;
...
HOMELOCATION="${INITRD_FLD}${PUP_HOME}${SAVEFILE}"
...
case ${EXIT} in save)
	...
	echo "KILOBIG=$KILOBIG" > ${INITRD_FLD}${PUP_HOME}/pupsaveresizenew.txt
	echo "PUPSAVEFILEX=$SAVEFILE" >> ${INITRD_FLD}${PUP_HOME}/pupsaveresizenew.txt #131231
